...
import { Button, Page, ProgressBar, Tab, Tabbar, Toolbar } from 'react-onsenui';
...
class EasySchedulerApp extends Component {
  ...
  loadCalendars = () => {
    ...
    getCalendars(this.state.gapi).then(res => {
      if (calendarId && res.result.items.length > 0) {
        const foundIdx = res.result.items.findIndex(calendar => calendar.id === calendarId);
  ...
  renderTabs = () => {
    ...
    return [
      {
        content: (
          <MyTab key="Schedule-c">{!calendarId ? <span>Välj en kalender i inställningar</span> : <DatePicker />}</MyTab>
        ),
      ...
      return (
        <Page contentStyle={{ display: 'flex', alignItems: 'center' }}>
          <div style={{ height: '300px' }}>
            ...
            {!loading && !signedIn ? (
              <Button style={{ marginTop: '50px' }} modifier="easyscheduler--outline" onClick={this.signIn}>
